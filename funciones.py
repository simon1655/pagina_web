...
def create_book_database(books: dict) -> Union[bool, str]:
    try:
        ...
        for title, details in books.items():
            cursor.execute('''
                INSERT OR REPLACE INTO books (isbn, title, author, categories, language, summary, cover_art_url, reseñas)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                details["isbn"], title, details["author"], ", ".join(details["categories"]), details["language"],
                details["summary"], details["cover_art_url"], details.get("reseñas", "")
            ))
